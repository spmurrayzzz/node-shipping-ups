...
function UPS(args) {
  ...
  $scope.buildFreightShipRequest = function(data, options, resource, callback) {
    soap.createClient(path.join(__dirname,  'wsdl', 'freight_shipping', 'FreightShip.wsdl'), {endpoint: 'https://' + hosts[$scope.options.environment] + resource.p}, function(err, client) {
      if(err) {
        return callback(err, null);
      }
  ...
  var resources = {
    ...
    freight_ship: {p: '/webservices/FreightShip', f: $scope.buildFreightShipRequest, r: $scope.handleFreightShipResponse, freight: true}
  };
