...
function UPS(args) {
  ...
  $scope.buildFreightShippingRequest = function(data, options, resource, callback) {
    soap.createClient(path.join(__dirname,  'wsdl', 'freight_shipping', 'FreightShipping.wsdl'), {endpoint: 'https://' + hosts[$scope.options.environment] + resource.p}, function(err, client) {
      if(err) {
        return callback(err, null);
      }
      ...
      client.ProcessFreightShipping(params, function(err, result) {
        if($scope.options.debug) {
          console.log(util.inspect(parser.toJson(client.lastRequest, {coerce: false, object: true, sanitize: false}), {depth: null}));
        }
        if(err) {
  ...
  $scope.handleFreightShippingResponse = function(data, callback) {
    if(data.Response.ResponseStatus.Code !== '1') {
      return callback(data.Response.Error, null);
    }
    return callback(null, data);
  ...
  var resources = {
    ...
    freight_rate: {p: '/webservices/FreightRate', f: $scope.buildFreightRateRequest, r: $scope.handleFreightRateResponse, freight: true},
    freight_shipping: {p: '/webservices/FreightShipping', f: $scope.buildFreightShippingRequest, r: $scope.handleFreightShippingResponse, freight: true}
  };
