...
function UPS(args) {
  ...
  $scope.buildFreightRateRequest = function(data, options, resource) {
    var clientParams = {

      //set soap connection data (mandatory values)
      host    : hosts[$scope.options.environment],
      path    : resource.p,
      wsdl    : path.join(__dirname,  'wsdl', 'freight_rate', 'FreightRate.wsdl')
      ...
      header  : [
        {
          'name'      : 'Username',
          'value'     : 'item_value',
          'namespace' : 'UPSSecurity/UsernameToken'
        },
        {
          'name'      : 'Password',
          'value'     : 'item_value',
        ...
        },
        {
          ...
          'value'     : 'item_value',
          'namespace' : 'UPSSecurity/ServiceAccessToken'
        }
      ]
      */
    };
...
//soap client options
    ...
    new easySoap.Client(clientParams, clientOptions);
    ...
    var params = {
      UPSSecurity: {
        ...
        ServiceAccessToken: $scope.options.access_key
      },
      Request: {
        RequestOption: data.request_option
      },
      ShipFrom: {
        Name: data.ship_from.name,
        Address: {
          AddressLine: [data.ship_from.address_line_1, data.ship_from.address_line_2, data.ship_from.address_line_3],
          City: data.ship_from.city,
          StateProvinceCode: data.ship_from.state_code,
          PostalCode: data.ship_from.postal_code,
          CountryCode: data.ship_from.country_code
        }
      },
      ShipTo: {
        Name: data.ship_to.name,
        Address: {
          AddressLine: [data.ship_to.address_line_1, data.ship_to.address_line_2, data.ship_to.address_line_3],
          City: data.ship_to.city,
          StateProvinceCode: data.ship_to.state_code,
          PostalCode: data.ship_to.postal_code,
          CountryCode: data.ship_to.country_code
        }
      },
      PaymentInformation: {
        Payer: {
          ...
          Address: {
            AddressLine: [data.payer.address_line_1, data.payer.address_line_2, data.payer.address_line_3],
            City: data.payer.city,
            StateProvinceCode: data.payer.state_code,
            PostalCode: data.payer.postal_code,
            CountryCode: data.payer.country_code,
      ...
      HandlingUnitOne: {
        Quantity: data.unit1.quantity,
        Type: {
          Code: data.unit1.code,
        }
      },
      Commodity: {
  ...
  $scope.handleFreightRateResponse = function(json, callback) {
  ...
  var resources = {
    ...
    freight_rate: {p: '/webservices/Rate', f: $scope.buildFreightRateRequest, r: $scope.handleFreightRateResponse, freight: true}
  };
  ...
  function buildResourceFunction(i, resources) {
    ...
    } else if(resources[i].freight === true) {
      return function(data, options, callback) {
        if(!callback) {
          callback = options;
          options = undefined;
        ...
        resources[i].f(data, options, resources[i]).then(function(res) {
